...
**Try.** Observe the pleural swing, the moving venous-inflow path and the highlighted steep segment of the RV-function curve. Read the RV preload-reserve tile, then inspect the PPV interpretability warning.
...
**Try.** Compare PEEP levels with the preset opening profile, then select Non-reopenable while keeping collapse and tissue compliance unchanged. Prone positioning can be explored as a separate, deliberately coarse transformation.

**Read.** In the recruiter, added pressure opens units and shares gas among more aerated lung; in the non-recruiter, the same pressure mainly distends the remaining open lung. The preset leaves maximum lung capacity at the 6 L default: collapse makes the accessible baby lung smaller, while reduced `clung` independently makes its aerated tissue less compliant. A separate supine thoracic load shifts resting pleural pressure without changing chest-wall compliance. Follow end-expiratory Ppl and PL, plateau pressure, closed fraction at end expiration and opening excursion, derived PVR, its wedge-dependent quality badge, RV/LV ratio, septal interaction and output together. Extreme PEEP can still generate plateau pressures outside a useful quantitative range and can invalidate the catheter interpretation of the wedge surrogate; prone response is directional rather than patient-specific. See [lung opening profiles](recruitment-and-ri.md), [pulmonary vascular resistance](pulmonary-vascular-resistance.md), [pulmonary artery wedge pressure](pulmonary-artery-wedge-pressure.md) and [ventricular interdependence](ventricular-interdependence.md).

The preset uses pressure-dependent opening without memory. Its potential reopenable share and opening midpoint are listed in the generated settings below. Scenario tests constrain end-expiratory Ppl and PL, EELV, plateau pressure, measured respiratory-system compliance, filling pressures and the RV phenotype together. Preserving one opening parameter alone is insufficient for validation.
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| recruitable baseline | 0.94 | -1.4 / 13.4 | 19.6 | 41.1% | 59% | 5.3 | 1.72 | 4.00 |
| recruitable, high PEEP | 1.30 | 0.4 / 19.6 | 41.0 | 25.7% | 74% | 5.8 | 1.77 | 3.78 |
| non-reopenable, high PEEP | 1.03 | -0.9 / 20.9 | 46.6 | 42.0% | 58% | 7.2 | 1.97 | 3.50 |
...
Read filling pressure with its reference explicit: atmospheric atrial pressure includes the pressure surrounding the chamber, whereas transmural pressure subtracts pleural and pericardial pressure. A higher measured CVP therefore need not mean greater right-heart filling. At settled PEEP levels, blood can redistribute from pulmonary vessels and cardiac chambers into systemic veins without any loss of total blood volume. Lower LV ejection pressure also does not guarantee lower net pressure–volume work: the area of the loop depends on both ejection and filling. Myocardial oxygen consumption is not represented. See [transmural pressure](transmural-pressure.md) and [ventriculo-arterial coupling](ventriculo-arterial-coupling.md).
...
**Separate the mechanism from the clinical demonstration.** At the same starting LV volume and fixed aortic pressure, increasing the pressure surrounding the ventricle lets it eject more blood in the model. This isolated, single-beat experiment holds the other compartments fixed and suppresses mitral filling, septal interaction and pericardial constraint. It tests afterload sensitivity; it is not a PEEP intervention in an intact circulation. The reproducible research probe is `tools/experiments/lv-afterload.mjs`.

In the intact circulation, distinguish the first response from the settled response. A PEEP step can temporarily raise aortic flow while right-heart output is already falling; pulmonary blood displacement and transit contribute, so that transient cannot be assigned to LV unloading alone. Once the circulation settles, filling and emptying must be assessed together. The current preset does not demonstrate a sustained output benefit from PEEP. A selected congested LV phenotype that does demonstrate that benefit remains an unresolved teaching objective; passing an isolated-mechanism check does not resolve it.

**Less filling is not automatically worse.** Relief of excessive filling pressure can be clinically useful, and maintaining output at a lower pressure burden is a different useful result from increasing output. Judge congestion, ejection load and forward flow separately. A single move to lower filling pressure does not establish an improved cardiac-function curve: that requires comparing flow at matched filling pressure, or the filling pressure needed for matched flow, with the same pressure reference. At whole-circuit steady state mean venous return equals mean cardiac output; a transient reduction in inflow and a maintained or increased LV output can coexist while blood redistributes. The [Guyton panel](panel-guyton.md) uses atmospheric right atrial pressure and a fast local RV-function relation by default. Its optional Deep CO mode measures a whole-heart response at the LV outlet. Curve position combines the represented loading effects and does not, by itself, measure a change in intrinsic LV contractility.
