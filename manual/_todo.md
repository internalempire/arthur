...
The isolated LV probe demonstrates greater forward ejection at fixed starting volume and aortic pressure, but the intact `lv-failure` preset loses enough filling to reduce settled output. The September 6 exploratory comparisons also test loading, passive LV stiffness, venous compliance, RV contractility and coupling. They do not establish impossibility across the control space or justify changing the preset solely to force a positive result. The September 23 controlled passive-VCV comparison documents filling, ejection pressure and conserved blood redistribution across two contractility and three baseline stressed-volume settings, with reversible PEEP sequences and timestep refinement. Greater available volume attenuates the output loss, but none of these conditions produces a sustained increase. Define a clinical comparison with explicit respiratory effort, pressure reference and measurement timing, then identify which physiological conditions remain unmatched before proposing a mechanistic or phenotype change. Retain independent flow integration, valid phases, plausible absolute pressures/volumes and numerical convergence as separate conditions. See `tools/experiments/lv-afterload.mjs` and the handover.
