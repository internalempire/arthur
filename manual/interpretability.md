...
The default Guyton panel updates local RV-function and venous-return formulas continuously, without running separate loading simulations. The trail uses one-heartbeat means; the measured venous-inflow marker and fast RV relation use respiratory means. VR live exposes instantaneous return determinants and withholds the mean-equilibrium crossing. Deep CO is an explicit opt-in that adds a separately settled whole-heart response and a measured mean LV-output marker. Its fixed autonomic drive and shared reference trajectories define a different analysis. Invalid or unsettled loading points are omitted; an invalid patient also withholds the LV-output marker and its text. Switching back, changing a parameter or resetting cancels the deep calculation. Neither the local RV crossing nor agreement at the deep reference point is independent clinical validation.
