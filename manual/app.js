...
let searchText = {};    // generated full-page text, keyed by slug
...
async function boot() {
  ...
  try {
    ...
    if (r.ok) searchText = (await r.json()).pages || {};
  } catch { /* title/summary search remains available as an offline fallback */ }
  flat = manifest.sections.flatMap((s) => s.pages.map(([slug, blurb]) => ({
    slug, blurb, section: s.label, title: pageTitle(slug), body: searchText[slug] || '',
  })));
...
function wireChrome() {
  ...
  q.addEventListener('input', () => {
    ...
    const hits = searchPages(flat, term, 12);
    results.innerHTML = hits.length
      ? hits.map((p) => `<li role="option"><a href="${manualHash(p.slug)}">${escapeHtml(p.title)}<small>${escapeHtml(p.excerpt || p.blurb)}</small></a></li>`).join('')
