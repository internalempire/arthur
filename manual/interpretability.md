...
The Guyton panel separates measured venous-inflow dynamics, measured mean LV output and a separately computed steady response. The trail uses one-heartbeat means; the two live markers use the most recent complete respiratory cycle after heartbeat smoothing. The ascending curve integrates LV outflow in whole-heart loading experiments at fixed autonomic drive and shared reference respiratory/source-pressure trajectories. The descending curve uses the same reference conditions. Their crossing is a local equilibrium construction, not independent clinical validation. Invalid or unsettled loading points are omitted; venous inflow must never be relabelled as LV output during redistribution.
