...
Controls are generated from the model's parameter registry. A grey control is not applicable in the current state—for example, tidal volume in pressure control or closing pressure while recruitment hysteresis is off.
...
| Guyton diagram | where do venous return and cardiac function meet, and is there preload reserve? | [Guyton diagram](panel-guyton.md) |
...
The two ventricles share a septum and a constrained space, described under [ventricular interdependence](ventricular-interdependence.md). Their output depends on filling, contractility and arterial load; [ventriculo-arterial coupling](ventriculo-arterial-coupling.md) provides the conceptual framework. [The four effects of a breath](the-four-effects-of-a-breath.md) puts these pathways in temporal order.
