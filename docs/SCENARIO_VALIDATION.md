...
| `lv-failure` | qualified | PEEP lowers transmural ejection pressure and filling; forward output falls modestly, with admissible cardiac phases. |
...
**What the model produces.** With PEEP 0 to 10, mean aortic output falls from approximately 3.12 to 2.97 L/min after settling. Both two-step-size audit runs show zero mitral-to-aortic overlap and zero late-diastolic aortic volume. Filling pressure remains elevated and EF remains reduced. Lower transmural ejection pressure therefore does not imply increased pumping.

**Assessment — qualified.** The preset demonstrates competition between reduced ejection pressure and reduced filling. Its parameters are a didactic phenotype, not a fitted patient or a general prediction for LV failure. The direct-flow and phase criteria in `tests/audit.mjs` are required for acceptance.
...
**What the model produces.** Against a comparator with normal wall compliance,
no wall load and normal abdominal pressure at the same tidal volume and PEEP,
the preset raises pleural swing from about 2.5 to 6.7 cmH2O, raises measured CVP
and lowers output. Low compliance produces the larger swing; the separate
positive wall load raises the resting pressure around the heart.

**Assessment — qualified.** The direction is sound, but the preset combines low
chest-wall compliance, a positive external wall load and raised abdominal
pressure and therefore represents a mechanical phenotype, not obesity itself.
Its PPV is under caution because the
...
**Not represented.** Body habitus, a measured relation from mass or abdominal
pressure to thoracic load, regional atelectasis, airway closure, diaphragm
geometry, position dependence and obesity-related cardiovascular disease.
...
makes the collapse-limited state worse. The preset also applies a selected
6 cmH₂O wall load, shifting resting pleural pressure through diaphragmatic
transmission without defining that load as chest-wall stiffness.
...
not represented separately and the splanchnic reservoir remains aggregate. The
thoracic load is not calculated from abdominal pressure; both controls are
selected explicitly because the model lacks the geometry needed to predict a
patient-specific transmission fraction.
