...
| `lv-failure` | needs correction | PEEP lowers transmural ejection pressure, but direct flow and phase audits invalidate the output-benefit claim. |
...
**What the model produces.** PEEP lowers LV transmural end-systolic pressure. However, the chamber-volume output estimate disagrees with directly integrated aortic flow; the latter falls in the PEEP 0-to-10 comparison. Simultaneous mitral inflow and aortic outflow during atrial contraction invalidates the intended normal cardiac phase sequence.

**Assessment — needs correction.** The pressure-unloading mechanism alone does not demonstrate improved pumping. Independent flow integration and diastolic-phase criteria in `tests/audit.mjs` are required for acceptance.
