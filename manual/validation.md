...
The audit tracks eight criteria: agreement of displayed cardiac output with integrated aortic flow; diastolic mitral-to-aortic throughflow; ventricular pressure increasing with activation at fixed volume; activation continuity at high heart rates; agreement of R/I with a finite-volume ventilator manoeuvre; hysteresis time-step convergence; preservation of intrinsic lung recruitability during a chest-wall intervention; and reverse flow through an open caval segment. Their executable status is recorded in [`audit-expectations.json`](https://github.com/internalempire/arthur/blob/main/tests/audit-expectations.json). The four cardiac criteria, open-caval reversal, hysteresis convergence and preservation of opening potential during chest-wall interventions are enforced as regression checks. The finite-volume R/I criterion remains open: the static research analogue is not validated as a ventilator manoeuvre, even when development checks complete. It is not a displayed clinical readout. Its reference PEEP step is <!-- CONSISTENCY: ri-reference-step -->5 → 15<!-- /CONSISTENCY --> cmH₂O and its compliance is a local tangent, not the ventilator's finite-tidal-volume ratio.

Caval regression checks require zero flow at equal pressures, flow in the pressure-gradient direction, preservation of the forward branch, and conserved volume during reversal, including when the atrial donor reaches its protective volume floor. Resolving this criterion does not validate human backflow amplitudes or remove the [remaining waterfall limitations](vascular-waterfalls.md#limits), including the near-equilibrium zero-flow interval and single caval closing pressure.
